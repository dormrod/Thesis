...
The weighting method used in this work is the so called radical tessellation, or power diagram \cite{GELLATLY1982,Aurenhammer1987}. In this modification, the dividing line is placed a distance $r_i$ from particle $i$, given by:
\begin{equation}
	\label{eq:radical}
	r_i = \frac{w_i^2-w_j^2+r_{ij}^2}{2r_{ij}}\,,
\end{equation}
...
This is because at the contact distance, $r_{ij} = R_i + R_j$, equation \eqref{eq:radical} shows that $r_i = R_i$ \ie{} the radical dividing line sits exactly between the two disks, producing the most equitable distribution of volume (see figure \ref{fig:vorodemor1}). 
Furthermore, when the radii are equal, $r_i = r_{ij} /2$ and the result from the standard unweighted Voronoi is regenerated as expected.
...
\davidnote{cite \cite{Rycroft2009} somewhere}
