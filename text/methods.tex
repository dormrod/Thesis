...
A system of interest can exist in a (very large) number of configurational states, $\left\{\vrc{0},\vrc{1},\cdots,\vrc{M}\right\}$ (in this work the notation $\vrc{i}$ will be used to denote all particle positions in a state $i$, whereas $\vrp{i}$ will be used to denote the $i$\th{} particle position in a given state).
...
\begin{enumerate}
	\item Calculate the potential energy of the system $\mathcal{U}_i=\mathcal{U}\left(\vrc{i}\right)$
	\item Determine the negative gradient of the potential \ie{} the forces acting on the particles $\mathbf{F}_i=-\nabla \mathcal{U}_i$ 
	\item Find the optimal distance to displace the particles along the lines of force $\mathcal{U}_{i+1}=\min \left[\mathcal{U}\left(\vrc{i}+\lambda \mathbf{F}_i\right)\right]$
	\item Set $\vrc{i+1}=\vrc{i}+\lambda_{\text{min}} \mathbf{F}_i$
...
For higher packing fractions a more sophisticated algorithm is needed where particles are swelled and optimised \cite{Woodcock1976}.
...
Most of these have either been covered in chapter \ref{ch:networktheory} or will be introduced as an accompaniment in the relevant chapters, but more elementary techniques are established here.
...
The RDF in \td{} is then given by:
\begin{equation}
	g\left(r\right)=\frac{1}{2 \mathcal{N} \rho \pi r}\left\langle \sum_i \sum_{j\neq i} \delta\left(r-r_{ij}\right)\right\rangle\,. 
\end{equation} 
The behaviour of the RDF is therefore such that $g\left(r\right)>1$ indicates an increase in probability that two points are at a given separation and $g\left(r\right)<1$ indicates a corresponding decrease.
...
In fact, partial RDFs will be used, to measure correlations between rings of different sizes.
