...
\begin{figure}[bt]
     ...
     \caption{Overview of the evolution in the partial ring statistics in bidisperse colloidal monolayers. Panels (a) and (b) show the partial mean sizes for the two radius ratios (as indicated in captions). Panels (c) and (d) show the proportion of hexagons in the partial distributions for the $\gamma=1.45$ systems. The blue curves correspond to quantities pertaining to large particles, red curves to small particles. The system composition is indicated by the depth of shading, as in the legends. Experimental points are coloured using the same scale as for simulation curves, and error bars indicate one standard deviation of the mean.}
     \label{fig:bi}
\end{figure}
