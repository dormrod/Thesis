...
Coupled with this, the Si\--Si distances remain in a relatively tight distribution, as shown in table \ref{tab:trsidist}.
...
\begin{enumerate}
	\item A cycle is born when a 1\--simplex cuts a previous cycle in two. The birth value is then equal to the filtration value of the 1\--simplex.
...
This is in fact the limiting case, and beyond this angle the circumcircle for $AD$ is not contained within that for $ACD$, and has a smaller radius, and therefore filtration value.
...
It has been stated that this analysis is not unique to the octagon, which has merely been selected as it \textit{can} display all the required behaviour.
...
For the most common ring, the hexagon, most easily forms secondary cycles in $B_3$, and to a lesser extend $B_2$. 
...
To conclude this section, the reasoning outlined above is tested by generating the bands numerically, by distorting polygons conforming to the various idealised models. 
The results are given in figure \ref{fig:blines}, which seems quite compelling.
...
As table \ref{tab:trsidist} showed, the nature of the triangle raft model (of hinged near\--rigid triangles), means that the bond lengths can only be less than the equilibrium value.
The properties of the triangle rafts have therefore enabled the structure of persistence diagrams to be well understood in this \td{} case.
