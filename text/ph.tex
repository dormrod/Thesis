...
Persistent homology is a technique in topological data analysis to find the fundamental topological features of collections of points.
...
The persistence aspect arises from the fact that these topological features can be calculated across different length scales, with those existing across multiple length scales being identified as more \textit{persistent}.
...
In the words of Wasserman: ``\textit{...the main purpose of TDA is to help the data analyst summarize and visualize
...
For systems with relatively rigid geometries, persistence diagrams have proved effective in showing the evolution in structure through the appearance of bands.
A series of bands, $B_i$, manifest from atoms separated by $i$ bonds.
These diagrams can then be considered to contain more information than other measures such as RDFs or structure factors, which as the interaction distances are already partitioned into increasing nearest\--neighbour contributions.
...
Presenting these data through the Betti numbers does allow a good estimation of the ring statistics in the system.
...
Overall, there is some cause for optimism for the use of persistent homology as another tool in the characterisation of amorphous materials.
...
Requiring only the atomic coordinates as an input, persistent homology delivers persistence diagrams, Betti numbers and lifetime distribution functions as outputs for analysis.
...
This information is not accessible from traditional methods such as radial  distribution functions, which describe pair\--correlations alone.
The introduction of new ring sizes could also be quantified by plotting the Betti numbers with filtration value.
On the other hand, the configurations from bond switching displayed similar features in the persistence diagrams, but these quickly aggregated with increased disorder to make quantifying the structure more difficult.
With  increasing  structural  disorder,  features in the Betti numbers broadened to the point where the persistent cycles and primitive rings no longer coincide.  
...
The findings for \td{} systems provide a starting point for the interpretation of persistent homology metrics for three\--dimensional systems.
For instance, it may be possible to follow processes such as crystallisation  in  which  the  ordering  increases, and  link  quantitatively  the  structural  evolution to the appearance of features persistence diagrams.
