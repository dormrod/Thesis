...
A consequence of these bridging oxygen atoms is to enforce a symmetry plane between the upper and lower layers. \davidnote{Put more bilayer stuff in intro?}
\marknote{Could do. It's ok here as background as it makes the chapter more self contained.}
...
Whilst this is on the surface the best solution, the experimental configurations may contain defects or areas where the image is corrupted \ie{} the configuration may not be ``pristine'' enough for use in computational studies. \marknote{Could lose the end of the sentence - implies calculations betters than experiments?}
...
This likely explains why this method has struggled to mirror experimentally observed values of the ring statistics and \aw{} parameter, with small and large rings being under\--estimated, \davidnote{as will be shown in sec x} \cite{Kumar2014}.
...
\davidnote{Accompanying figure?}
\marknote{Probably a good idea.}
...
Below this point, the entropy of \marknote{the?} $\br{4}{10}$ system decreases whilst the energy continues to rise and so there is no driving force to sample this area of phase space.
...
This change in area with distortion is shown in figure \ref{fig:trellipse}, the calculation of which can be found in appendix \davidnote{add ellipse appendix}. 
