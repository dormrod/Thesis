...
The realisation of these materials has posed additional challenges.
...
However, \lm's law does not give explicit information on why a particular maximum entropy distribution is found for a given system.
This is because whilst the underlying constraints should correspond to measurable structural properties in the material, as discussed in section \ref{s:lemaitre}, the physical basis of one constraint is unclear and may vary between different materials.
...
One could also define a free energy for this system, $\mathcal{F}=\mathcal{U}-T\mathcal{S}$, and investigate its behaviour.
The problem is that it is not clear what temperature would most reasonably correspond to the experimental system. 
This is because the \mc{} temperature parameter does not correspond to a true physical observable, and the relationship between the ring entropy and the configurational entropy is not yet well defined.
Therefore whilst one can demonstrate that there exists a temperature which will give a minimum in $\mathcal{F}$, at $p_6\approx 0.4$, it is not currently feasible to rigorously tie this to experimental conditions.
The interesting possibilities arising from this relationship however warrant further investigations.
These could help to determine a temperature window over which these ring statistics would be predicted to be found, and therefore whether the experimental samples are consistent with those expected from equilibrium thermodynamics. 
...
There are additional subtle effects that may be captured by moving to an all\--atom model, and relaxing the constraint of strictly\--enforced two\--dimensionality.
For instance, geometrically frustrated materials can adopt entropy\--driven ordered configurations at finite temperatures, in a process termed ``order by disorder'' \cite{Shokef2011}.
Zig\--zag patterns have been found to occur starting from an antiferromagnetic Ising model on a triangular lattice, and in \td{} colloids, as a result of buckling of the monolayer \cite{Han2008}.
It is possible that analogous processes could occur in bilayer materials, which  would only be observable by adopting a more detailed model.
